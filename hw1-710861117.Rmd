...
```{python ans12}
x_str = [str(x[i]) for i in range(0,12)]
x_str
```
...
```{python ans14}
x_logi = [x[i]<50 for i in range(0,12)]
x_logi
```
...
```{python ans15}
num_false = len(x_logi)-sum(x_logi)
num_false
```
...
```{python ans21}
logi = ["桃園市" in dict_business["公司所在地"][i] for i in range(len(dict_business["公司所在地"]))]
...
```{python ans22}
logi2 = [dict_business["資本額"][i]>500000 for i in range(len(dict_business["資本額"]))]
...
local1 = showInfo[0].get("location")
local1
```
