...
```{python ans12}
index_3ticks = [i for i, x in enumerate(filelines) if "```" in x]
...
```{python ans13}
start = [i+1 for i, x in enumerate(filelines) if "```{" in x]
end = [i-1 for i, x in enumerate(filelines) if x == "```"]
begin = [i for i, x in enumerate(filelines) if "```{" in x]
code = [filelines[i] for i in begin]
...
```{python ans14}
number = []
start_p = [i+1 for i, x in enumerate(filelines) if "```{p" in x]
true_end = [i for i, x in enumerate(filelines) if x == "```"]
true_end_p = true_end[1:len(true_end)]

for i in range(len(start_p)):
    number.extend([i for i in range(start_p[i],true_end_p[i])])

python_code_list = [filelines[number[i]] for i in range(len(number))]
...
dict1 = {
    ...
    'location': '新北市三峽區大學路151號', 
    'locationName': '國立臺北大學'
    }
updated_result.append(dict1)
