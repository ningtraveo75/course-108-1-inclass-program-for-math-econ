...
```{python ans12}
marginP_weather = np.sum(df_jointP,axis=1)
marginP_mood = np.sum(df_jointP,axis=0)
...
```{python ans13}
P_weather_cond_mood = pd.DataFrame()
for i in range(0,3):
    P_weather_cond_mood[i,] = df_jointP.iloc[i].divide(marginP_weather[i])
P_weather_cond_mood.set_axis(hw3data['weather index'],axis=1,inplace=True)
P_weather_cond_mood = np.transpose(P_weather_cond_mood)

P_mood_cond_weather = pd.DataFrame()
for j in range(0,4):
    P_mood_cond_weather[j,] = df_jointP.iloc[0:3,j].divide(marginP_mood[j])
P_mood_cond_weather.set_axis(hw3data['mood index'],axis=1,inplace=True)
...
```{python ans14}
p_ans14 = P_weather_cond_mood['sad']['sunny']
...
def x_star(x_partition, alpha=0.5):
    ans1 = [x_partition[i]+alpha*(x_partition[i+1]-x_partition[i]) for i in range(len(x_partition)-1)]
    return ans1
...
```{python ans23}
ans=[]
x_partition = np.linspace(a,b,num=100)
x_star_ans = x_star(x_partition,)

for i in range(len(x_partition)-1):
    c = f(x_star_ans[i])*(x_partition[i+1]-x_partition[i])
    ans.append(c)
rsum = sum(ans)
...
def riemannsum(fun,interval,N=101,alpha=0.5):
    ...
    x_partition = np.linspace(interval[0],interval[1],num=N)
    
    x_star_ans = [x_partition[i]+alpha*(x_partition[i+1]-x_partition[i]) for i in range(len(x_partition)-1)]
